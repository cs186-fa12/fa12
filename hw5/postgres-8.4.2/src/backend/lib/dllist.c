...
/*
 * Add the element "e" to the same list that contains "after", one position
 * after it.
 */
void
DLAddAfter(Dlelem *after, Dlelem *e)
{
	Assert(after->dle_list != NULL);

	e->dle_list = after->dle_list;
	e->dle_next = after->dle_next;
	e->dle_prev = after;

	if (after->dle_next)
		after->dle_next->dle_prev = e;
	else
	{
		Assert(e->dle_list->dll_tail == after);
		e->dle_list->dll_tail = e;
	}
