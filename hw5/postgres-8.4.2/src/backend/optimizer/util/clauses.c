...
typedef struct
{
	int			approx_agg_count;
	int			nkeep;
	int			cm_width;
	int			cm_depth;
	bool		seen_other_aggs;
} approx_agg_walker_context;

static bool contain_agg_clause_walker(Node *node, void *context);
static bool pull_agg_clause_walker(Node *node, List **context);
static bool count_agg_clauses_walker(Node *node, AggClauseCounts *counts);
static bool check_approx_agg_usage_walker(Node *node, void *data);
...
/*
 * NB186: This function walks an expression tree and checks if it references
 * the approximate count aggregate function. If so, applies a few error
 * checks and returns "true".
 */
bool
check_approx_agg_usage(Node *node, int *approx_nkeep, int *cm_width, int *cm_depth)
{
	approx_agg_walker_context context;

	MemSet(&context, 0, sizeof(context));

	check_approx_agg_usage_walker(node, &context);

	if (context.approx_agg_count > 0 && context.seen_other_aggs)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("cannot reference both approx_count and "
					 "other aggregates in the same target list")));

	if (context.approx_agg_count > 0)
	{
		*approx_nkeep = context.nkeep;
		*cm_width = context.cm_width;
		*cm_depth = context.cm_depth;
		return true;
	}
...
static bool
check_approx_agg_usage_walker(Node *node, void *data)
{
	approx_agg_walker_context *context = (approx_agg_walker_context *) data;
	...
	if (IsA(node, Aggref))
	{
		...
		if (agg->aggfnoid == APPROX_COUNT_AGG_OID)
		{
			Const *nkeep;
			Const *cm_width;
			Const *cm_depth;

			context->approx_agg_count++;

			if (context->approx_agg_count > 1)
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("cannot reference approx_count multiple "
							 "times in the same target list")));

			/*
			 * Should be guaranteed by the semantic analysis phase, but
			 * we're paranoid
			 */
			if (list_length(agg->args) != 3)
				elog(ERROR, "unexpected number of arguments to approx_count: %d",
						list_length(agg->args));

			if (!IsA(linitial(agg->args), Const) || !IsA(lsecond(agg->args), Const) ||
				!IsA(lthird(agg->args), Const))
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						 errmsg("the arguments to approx_count must be constants")));

			nkeep = (Const *) linitial(agg->args);
			cm_width = (Const *) lsecond(agg->args);
			cm_depth = (Const *) lthird(agg->args);

			if (nkeep->constisnull || cm_width->constisnull || cm_depth->constisnull)
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						 errmsg("the arguments to approx_count must not be NULL")));

			context->nkeep = DatumGetInt32(nkeep->constvalue);
			context->cm_width = DatumGetInt32(cm_width->constvalue);
			context->cm_depth = DatumGetInt32(cm_depth->constvalue);

			if (context->nkeep <= 0 || context->cm_width <= 0 || context->cm_depth <=0)
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						 errmsg("the arguments to approx_count must be positive")));

			// if (context->nkeep > context->nslots)
			// 	ereport(ERROR,
			// 			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			// 			 errmsg("the number of results to keep from approx_count "
			// 				 "must not exceed the number of slots"),
			// 			 errdetail("Number of results to keep: %d. "
			// 				 "Number of slots: %d.",
			// 				 context->nkeep, context->nslots)));
		}
		else
			context->seen_other_aggs = true;

		return false;
	}

	return expression_tree_walker(node, check_approx_agg_usage_walker, data);
}
