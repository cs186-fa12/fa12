...
make_agg(PlannerInfo *root, List *tlist, List *qual,
		 ...
		 int approx_nkeep, int cm_width, int cm_depth,
		 Plan *lefttree)
{
	Agg		   *node = makeNode(Agg);
	Plan	   *plan = &node->plan;
	long		numOutputGroups;
	Path		agg_path;		/* dummy for result of cost_agg */
	QualCost	qual_cost;

	if (aggstrategy == AGG_HASHED_APPROX)
		numOutputGroups = Min(numGroups, approx_nkeep);
	else
	{
		numOutputGroups = numGroups;
		Assert(approx_nkeep == 0);
		Assert(cm_width == 0);
		Assert(cm_depth ==0);
	}
	...
	node->numGroups = numGroups;
	node->approx_nkeep = approx_nkeep;
	node->cm_width = cm_width;
	node->cm_depth = cm_depth;

	copy_plan_costsize(plan, lefttree); /* only care about copying size */
	cost_agg(&agg_path, root,
			 aggstrategy, numAggs,
			 numGroupCols, numOutputGroups,
	 ...
	 * output group otherwise.
	 */
	if (aggstrategy == AGG_PLAIN)
		plan->plan_rows = 1;
	else
		plan->plan_rows = numOutputGroups;
