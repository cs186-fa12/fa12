...
static bool choose_hashed_grouping(PlannerInfo *root,
					   ...
					   double dNumGroups, bool force_hashing, AggClauseCounts *agg_counts);
...
grouping_planner(PlannerInfo *root, double tuple_fraction)
{
	...
	else
	{
		...
		if (parse->hasAggs)
		{
			...
			use_approx_agg = check_approx_agg_usage((Node *) tlist,
				&approx_nkeep,
				&cm_width,
				&cm_depth);
			if (use_approx_agg && numGroupCols == 0)
			  ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					errmsg("approximate aggregation cannot be used "
		...
		if (parse->groupClause)
		{
			...
			/*
			 * NB186: If using approximate counting, we require that hashed
			 * aggregation be used; if that's not possible (e.g. because there
			 * is no hashing operator defined for one of the data types we're
			 * grouping on), we raise an error.
			 */
			if (can_hash && can_sort)
			{
				...
				use_hashed_grouping =
					choose_hashed_grouping(root,
										   ...
										   dNumGroups, use_approx_agg, &agg_counts);
			}
			else if (can_hash)
		...
		else
		{
			 ...
			 *
			 * NB186: If we're using approximate counting, use the
			 * AGG_HASHED_APPROX strategy.
			 */
			if (use_approx_agg)
			{
			  result_plan = (Plan *) make_agg(root,
				...
				/*
				 * NB186: We consider the output of the approximate agg to
				 * be unsorted, which is wrong.
				 */
				current_pathkeys = NIL;
			}
					   ...
					   double dNumGroups, bool force_hashing, AggClauseCounts *agg_counts)
{
	...
	if (!enable_hashagg) {
	  if (force_hashing)
		elog(ERROR, "planning error; hashing is disabled "
			"but it is required by the query");
	  return false;
	}
	...
	if (hashentrysize * dNumGroups > work_mem * 1024L) {
	  if (force_hashing) {
		/*
		 * NB186: Postgres avoids hashing if it looks like the hashtable will
		 * not fit in memory. If we have force_hashing then we are using
		 * approximate hashing which we assume will always fit in memory 
		 */
		return true;
	  }
	  return false;
	...
	/*
	 * NB186: The cost-based query optimizer found that sorting would be cheaper
	 * than hashing. However, if we have force_hashing then we are using
	 * approximate hashing which we assume will always be cheaper for this
	 * assignment. 
	 */
	if (force_hashing) {
	  return true;
