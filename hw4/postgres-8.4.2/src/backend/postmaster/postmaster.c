...
PostmasterMain(int argc, char *argv[])
{
	 ...
	 */
	while ((opt = getopt(argc, argv, "A:B:c:D:d:EeFf:g:h:ijk:lN:nOo:Pp:r:S:sTt:W:-:")) != -1)
	{
		switch (opt)
		{
			case 'A':
	...
	elog(LOG, "using buffer replacement policy: %s",
		 get_buffer_policy_str(BufferReplacementPolicy));
